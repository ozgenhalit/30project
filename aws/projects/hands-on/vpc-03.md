...
- Launch two Instances. First instance will be in "stevie-az1a-private-subnet" of "stevie-vpc-a",and the other one will be in your "Default VPC". 
...
- Select "stevie-private-rt" ---> Routes ----> Edit Rule ---> Add Route
```
Destination     : 0.0.0.0/0
Target ----> Nat Gateway ----> stevie-nat-gateway-2
```
- click save routes
...
### E. Configure Private instance in 'stevie-az1a-private-subnet' of 'stevie-vpc-a'.

```text
AMI             : Amazon Linux 2
Instance Type   : t2.micro
Network         : stevie-vpc-a 
Subnet          : stevie-az1a-private-subnet
user data       : 
...
```
Destination: paste "stevie-vpc-a" CIDR blok
Target ---> peering connection ---> select 'First Peering' ---> Save routes
```

- select stevie-private-rt's route table ----> Routes ----> 
Edit routes
```
...
- Create a bucket of `stevieway-vpc-endpoint` with following properties, 
...
Role Name : stevieS3FullAccessforEndpoint
Role description: stevie S3 Full Access for Endpoint
click create button
```
Go to EC2 service from AWS console

Select "Private WEB EC2" ---> Actions ---> Security ---> Modify IAM Role  select newly created IAM role named 'stevieS3FullAccessforEndpoint' ---> Apply
...
- list the bucket in S3 and content of S3 bucket named "aws s3 ls "stevieway-vpc-endpoint" via following command

```
aws s3 ls
aws s3 ls stevieway-vpc-endpoint
```
- go to private route table named "stevie-private-rt" on VPC service
...
- Go to private route table named 'stevie-private-rt' and show the endpoint rule that is automatically created by AWS 

### B. Connect  to S3 via Endpoint

- Go to terminal, list the buckets in S3 and content of S3 bucket named "aws s3 ls 
"stevieway-vpc-endpoint" via following command
```bash
aws s3 ls
aws s3 ls stevieway-vpc-endpoint
```
...
```bash
aws s3 cp s3://stevieway-vpc-endpoint/Guile.png .
aws s3 cp s3://stevieway-vpc-endpoint/Honda.png .
```
